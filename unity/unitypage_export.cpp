...
void UnityPage::exportSr()
{
    ...
    QWebElement ex = mViewFrame->findFirstElement( "select#s_at_m_SWEApplet2" );
    QWebElementCollection c = ex.findAll( "*" );
    QWebElement ele;
  
    for ( int i = 0; i < c.count(); ++i )
    {
        if ( c.at( i ).toInnerXml().contains( "Export.." ) )
        {
            js += c.at(i).attribute( "value" ) + ")";
        }
    }
